...
#I didn't use 'do' or 'tidy', but I think it worked...
...
projects %>%
  ...
  group_by(institute) %>%
  summarize(correlation = cor(n.pubs, total.cost))
```
...
projects %>%
  ...
  group_by(institute) %>%
  summarize(correlation = cor(total.cost, rcr))
...
#I tried, but I can't seem to install ImageMagick, so I keep getting error messages. I'm able to see the graph, but without animation. Also, the curve of these graphs since 2010 is rather depressing! 
